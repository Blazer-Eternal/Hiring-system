...
exports.getJobFilter = getJobFilter;
exports.applicationKey = applicationKey;
exports.mightBeDuplicate = mightBeDuplicate;
...
class BloomFilter {
     ...
     */
    mightContain(key) {
        return (this.getBit(hash1(key, this.size)) &&
    ...
    /** Restore from base64 */
    static deserialize(data, size = 10000) {
...
const jobFilters = new Map();
function getJobFilter(jobId) {
    if (!jobFilters.has(jobId)) {
        jobFilters.set(jobId, new BloomFilter(10000));
    }
 ...
 */
function applicationKey(candidateId, jobId) {
 ...
 */
function mightBeDuplicate(candidateId, jobId) {
 ...
 */
function registerApplication(candidateId, jobId) {
    const filter = getJobFilter(jobId);
